...
# 2. CSS για Animations (Fade και Slide)
css_code = "<style>"
css_code += ".stApp { background-color: #f0f7ff; }"

# Animation για την είσοδο της νέας ερώτησης (Fade In)
css_code += "@keyframes fadeIn { 0% { opacity: 0; transform: translateY(10px); } 100% { opacity: 1; transform: translateY(0); } }"

# Animations για αρχή και τέλος
...
# Εφαρμογή Fade In σε κάθε νέα κάρτα
css_code += ".fade-anim { animation: fadeIn 0.5s ease-out forwards; }"
...
# Δομή κάρτας (Χωρίς 3D Rotate πλέον για αποφυγή glitches)
css_code += ".card-content { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; border-radius: 25px; font-size: 55px; font-weight: bold; box-shadow: 0px 8px 16px rgba(0,0,0,0.1); border: 4px solid; transition: all 0.3s ease; }"
...
# --- ΟΘΟΝΗ ΠΑΙΧΝΙΔΙΟΥ ---
    ...
    if st.session_state.is_finished:
        ...
        # Επιλογή Στυλ (Ερώτηση ή Απάντηση)
        ...
        # Κατασκευή HTML (Fade Logic)
        # Χρησιμοποιούμε το card_id στο ID του div για να ξέρει ο browser ότι είναι "νέα" οντότητα
        card_html = '<div class="main-card ' + anim_class + '" id="q_' + str(st.session_state.card_id) + '_' + str(st.session_state.show_answer) + '">'
        ...
        if not st.session_state.show_answer:
            ...
            if c1.button("Το βρήκες! ✅", use_container_width=True):
                ...
                st.session_state.current_q = None # Αυτό θα προκαλέσει αλλαγή card_id στο επόμενο run
                st.rerun()
